...
const initializeDBAndServer = async () => {
  try {
    db = await open({
      // beacuse that await open() will return the promise connection object so we need to store it so we are using db
      filename: dbPath,
...
app.post('/books/', async (request, response) => {
  // to aceess the request objects from the http body where the data is stored...
  const bookdetails = request.body
  // doing object destructuring to get that all the variables or values which we want.. if not available then undefined values will give...
